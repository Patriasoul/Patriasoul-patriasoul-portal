document.addEventListener('DOMContentLoaded', () => {
  ...
  if (nav) {
    // Gradovi i Baština više nisu zasebne stavke glavne navigacije.
    // Gradovi ostaje pod Domovina, a Baština pod Povijest.
    nav.querySelectorAll(':scope > a').forEach(link => {
      const label = link.textContent.trim();
      if (label === 'Gradovi' || link.getAttribute('href') === '/gradovi.html' || label === 'Baština' || link.getAttribute('href') === '/bastina.html') {
        link.remove();
      }
    });
